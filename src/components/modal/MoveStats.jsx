...
const MoveStats = ({power, pp, accuracy, priority, type, contest, damage}) => { 
    const { type: pokemonType } = usePokemonContext();

    return (
        <div className='w-full flex flex-col gap-3'>
            <div className='w-full flex items-center justify-center
            flex-wrap gap-3'>
                {type && <p className={`p-2 rounded-lg
                capitalize ${pokemonType && `${types[pokemonType].backgroundColor} text-white`}`}>
                    {type}
                </p>}
                {damage && <p className={`p-2 rounded-lg
                capitalize ${pokemonType && `${types[pokemonType].backgroundColor} text-white`}`}>
                    {damage}
                </p>}
                {contest && <p className={`p-2 rounded-lg
                capitalize ${pokemonType && `${types[pokemonType].backgroundColor} text-white`}`}>
                    {contest}
                </p>}
            </div>
